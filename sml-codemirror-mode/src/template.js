CodeMirror.defineMode("%%%modename%%%", function(config, parserConfig) {
	...
	function getTerminals()
	{
        ...
        var SINGLELINE_COMMENT_BODY = '([^\\n\\r]*[\\n\\r]|[^\\n\\r]*$)';

//		var COMMENT1 = '#' + COMMENT_BODY;
//		var COMMENT2 = '//' + COMMENT_BODY;

        var SINGLELINE_COMMENT = '((#|(//))' + SINGLELINE_COMMENT_BODY + ')';

		// TODO Multiline comments are only working for a single line like that (oh the irony) - How to make them work?
		var MULTILINE_COMMENT = '(/\\*[\\s\\S]*\\*/)';

        var COMMENT = '(' + SINGLELINE_COMMENT + '|' + MULTILINE_COMMENT + ')';

		var WS_OR_COMMENT_STAR = '('+WS+'|('+COMMENT+'))*';
		var NIL   = '\\('+WS_OR_COMMENT_STAR+'\\)';
		var ANON  = '\\['+WS_OR_COMMENT_STAR+'\\]';

		var VAR_STRING = '(\\?|\\$)(' + STRING_LITERAL1 + '|' + STRING_LITERAL2 + ')';
		...
		var terminals=
			{
				terminal: [
					...
					{ name: "SQL_STRING",
						regex:new RegExp("^"+SQL_STRING),
						style:"sp-prefixed" },

					{ name: "VAR_STRING",
						regex:new RegExp("^"+VAR_STRING),
						style:"sp-var" },
...
//			else if (topSymbol=="allowNames") state.allowNames=true;
//			else if (topSymbol=="disallowNames") state.allowNames=false;
		}
		...
		function checkSideConditions(topSymbol) {
			...
			var b = (state.allowNames || topSymbol != "name");
			var result = a; // && b;
			return result;
		}
...
//		while(state.stack.length>0 && token && state.OK && !finished ) {
		while(state.stack.length>0 && token && !finished ) {
...
//console.log('Failed at state', state);
					state.OK=false;
					state.complete=false;
					recordFailurePos();
				}
